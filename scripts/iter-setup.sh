...
echo "  Opening GitHub. The scope is ALREADY selected: read:packages, and nothing else."
...
LOG="$(mktemp)"; PULL_OK=0
for i in 1 2 3; do
  if docker pull "$IMAGE" 2>&1 | tee "$LOG"; then PULL_OK=1; break; fi
  grep -qiE 'unauthorized|denied|forbidden' "$LOG" && break
  [ "$i" -lt 3 ] && { echo; echo "  transport error — retrying ($i/3)…"; sleep 3; }
done

if [ "$PULL_OK" -ne 1 ]; then
  echo
  if grep -qiE 'unauthorized|denied|forbidden' "$LOG"; then
